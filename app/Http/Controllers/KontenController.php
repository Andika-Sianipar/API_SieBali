...
class KontenController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if ($request->hasFile('gambar_konten')) {
            ...
            $extension = $file->getClientOriginalExtension();
            $fileName = $tanggalSekarang . '.' . $extension;
            $path = public_path('images'); // Menentukan path ke direktori public/images
            $file->move($path, $fileName); // Memindahkan file ke direktori public/images
            $data['gambar_konten'] = $fileName; // Menyimpan path relatif ke gambar di database
        }
    ...
    public function update(Request $request, $id)
    {
        ...
        if ($request->hasFile('gambar_konten')) {
            ...
            if (Storage::exists('public/' . $konten->gambar_konten)) {
                Storage::delete('public/' . $konten->gambar_konten);
            }
